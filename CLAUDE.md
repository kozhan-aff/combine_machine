...
## Текущее состояние
**Скелет + разведка интеграций.** Есть рабочий каркас FastAPI, конфиг со всеми env, реальные модели, стабы всех
интеграций (с деталями API в докстрингах), стабы сервисов, `smoke.py`, docs/. Все интеграции разведаны и
задокументированы — `docs/api/` (11 референсов + индекс, многое проверено вживую). Бизнес-логика — TODO.

## Что делать первым
**Фаза 0 — каркас зелёный:** `docker compose up` → `/health`; настроить Alembic + первая миграция;
реализовать `ping()` в клиентах и довести `scripts/smoke.py` до зелёного. Стартовый набор ping (креды НЕ нужны):
wayback, rkn (antizapret), searxng, litellm, openpagerank, backorder-discovery. Дальше не идти, пока не позеленеет.
**Фаза 1 — MVP:** M1 целиком (discovery + скоринг по docs/DONORS.md на free-стеке + API + список) → петля на ОДНОМ
домене (M3→M4→M5), покупка руками. Критерий MVP: сайт поднят системой, контент отредактирован, опубликован, страницы в индексе (проверка `site:`).
