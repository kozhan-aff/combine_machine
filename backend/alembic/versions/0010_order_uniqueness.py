...
и НЕ одинаково:
  ...
  · лишний `ordering`/`ordered` — деньги МОГЛИ уйти: закрываем в `failed` + `maybe_sent`, то есть
    в «исход неизвестен». Это не сокрытие: `failed` опрашивается poll_orders (заказ найдётся у
    провайдера по elid), отмена такого заказа запрещена, а «↻ повторить» сперва спрашивает
    find_order. Запись и provider_order_id остаются на месте.
