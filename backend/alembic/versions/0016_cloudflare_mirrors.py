...
переносящаяся страница на самом деле дублирует то, что уже есть у keeper — такую коллизию удаляем,
а не переносим (перенос лёг бы в тот же уникальный индекс и уронил миграцию).
