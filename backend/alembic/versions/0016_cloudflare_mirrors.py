...
ДЕТИ РАНЬШЕ РОДИТЕЛЕЙ (ревью Задачи 1, F-migration-0016 Critical #1/#2). Два FK в проекте объявлены
БЕЗ ondelete (= NO ACTION), и PostgreSQL их энфорсит: `index_history.page_id -> pages.id` и
`site_offers.site_id -> sites.id`. У published-дубля-страницы index_history есть штатно
(`publish.check_index` пишет строку для КАЖДОЙ published-страницы, тот же факт, что уже решался в
0014); у сайта-проигравшего site_offers тоже штатны (M4 привязывает офферы к сайту при генерации
контента). DELETE родителя с живыми детьми поднимает ForeignKeyViolation и откатывает ВСЮ
транзакцию миграции — обрывает git-pull-деплой ровно там, где дедуп должен был его спасти. Живой
Postgres-репро имплементера прошёл только потому, что в фикстуре не было ни одной строки
index_history/site_offers. Порядок: index_history коллизионных страниц удаляется ДО самих страниц;
site_offers сайтов-проигравших переносится на keeper ДО удаления сайтов.
...
def upgrade() -> None:
    ...
    #
    # Дети раньше родителей (ревью Задачи 1, F-migration-0016 Critical #1/#2): и
    # `index_history.page_id -> pages.id`, и `site_offers.site_id -> sites.id` объявлены БЕЗ
    # ondelete (= NO ACTION), PostgreSQL их энфорсит. У published-дубля index_history есть штатно
    # (publish.check_index пишет строку для КАЖДОЙ published-страницы, см. 0014); у
    # сайта-проигравшего site_offers тоже штатны (M4 привязывает офферы к сайту при генерации).
    # DELETE родителя с живыми детьми поднимает ForeignKeyViolation и откатывает ВСЮ транзакцию
    # миграции, обрывая git-pull-деплой — тот самый провал, ради защиты от которого дедуп и
    # написан. Поэтому: сначала index_history коллизионных страниц, потом сами страницы; сначала
    # site_offers сайтов-проигравших переносятся на keeper, потом сами сайты.
    op.execute("""
        WITH ranked AS (
        ...
        WHERE page_id IN (SELECT page_id FROM page_dupes WHERE dup_rank > 1);
    """)
    op.execute("""
        WITH ranked AS (
            SELECT s.id, s.domain_id,
    ...
    op.execute("""
        ...
        UPDATE site_offers SET site_id = r.keeper
        FROM ranked r WHERE site_offers.site_id = r.loser AND r.loser <> r.keeper;
    """)
    op.execute("""
        WITH ranked AS (
            SELECT s.id AS loser, s.domain_id,
