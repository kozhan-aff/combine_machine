...
class BackorderClient(BaseClient):
    ...
    def pick_tariff(self, domain: str, bid_rub: float) -> dict:
        ...
        в заказе, и execute уже ничего не «до-решает» за человека. Ставка выше сетки ->
        верхний тир (платить больше максимума нечем). Незнакомая зона / пустая сетка ->
        RuntimeError: молча заказать по чужому тарифу = списать не те деньги.
        """
        zone = zone_of(domain)
        if zone is None:
            raise RuntimeError(
