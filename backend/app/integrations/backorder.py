...
# Не перечисленные (2 «не оплачен», 4, 5, 10, 13) — заказ ещё в полёте: pending.
_STATE = {
    8: "caught",    # домен в процессе передачи — ведёт к 11
    11: "caught",   # завершён: домен пойман
    3: "failed",    # перекрыт
...
def zone_of(domain: str) -> str:
    """Зона тарифной сетки домена. .РФ приходит кириллицей (фид), но punycode тоже ловим."""
    d = (domain or "").strip().lower()
    return ".РФ" if d.endswith(".рф") or d.endswith(".xn--p1ai") else ".RU"
...
class BackorderClient(BaseClient):
    ...
    def _billmgr(self, func: str, *, retry: bool = True, **params) -> list[dict]:
        ...
        try:
            if retry:
                resp = self.request("GET", _BILLMGR, params=p)
            else:
                resp = self._client.request("GET", _BILLMGR, params=p)
    ...
    def balance(self) -> float | None:
        ...
        elem = self._billmgr("accountinfo")
        if not elem:
            return None
        try:
            return float(elem[0].get("balance"))
        except (TypeError, ValueError):
            return None
    ...
    def pick_tariff(self, domain: str, bid_rub: float) -> dict:
        ...
        Ставка выше сетки -> верхний тир (платить больше максимума нечем). Пустая сетка
        -> RuntimeError: молча заказать по неизвестному тарифу = потратить не те деньги.
        """
        zone = zone_of(domain)
        grid = self.tariffs(zone)
        if not grid:
