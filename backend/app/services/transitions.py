...
ЧТО ЗДЕСЬ ЛЕЖИТ. Два запрета, оба — про РУЧНЫЕ действия:
  ...
  2. грязный домен (`dirty_reason`) не входит в статусы, ведущие к деньгам, — никаким ручным
     действием. Мимо этого запрета не пройти и в M2: `create_order`/`confirm_order` спрашивают
     `dirty_reason` отдельно (заявка на грязный домен могла быть заведена ДО этого фикса).
