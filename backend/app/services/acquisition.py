...
def _settle(db, order_id: int, was: str, **values) -> bool:
    """Записать исход строки УСЛОВНЫМ UPDATE'ом: только если она всё ещё в том статусе, в котором
    мы её застали. True — записали; False — строку у нас увели, и мы не пишем НИЧЕГО.

    ЗДЕСЬ БЫЛ ПОСЛЕДНИЙ СЛЕПОЙ ПИСАТЕЛЬ ПО PK (ревью Задачи 8). Поллинг судит по СНИМКУ: строки
    выбираются одним SELECT'ом ДО цикла, а пишутся по одной, с коммитом на каждой (SessionLocal
    живёт с expire_on_commit=False — снимок сам не освежается). За это время `failed`-строку
    успевает заново заклеймить живой execute («↻ повторить») и довести до конца: в БД она уже
    'ordering' или 'ordered' с ОПЛАЧЕННЫМ заказом. ORM-запись по первичному ключу (UPDATE ...
    WHERE id=N) об этом не спрашивает — она пишет то, что решил протухший снимок, и сажает на
    оплаченную строку elid ЧУЖОГО, мёртвого заказа (старая `failed`-запись провайдера, «Перекрыт»
    с прошлого цикла). Дальше цепочка достраивается сама: следующая сверка судит строку по этому
    elid → 'failed' → в очереди снова «↻ повторить» → find_order remote-`failed` игнорирует
    (integrations/backorder.py) → ВТОРОЙ платный заказ, пока первый в полёте.

    Условие в SQL делает решение и запись одним атомарным актом: пишет не тот, кто прочитал, а
    тот, кто застал строку на месте. Тот же приём, что в `cancel_order` и в claim'е `execute`
    (аудит F12) — гонку SELECT-гардом не закрыть, это разные запросы.
    """
    from sqlalchemy import update
    from app.models.domain import AcquisitionOrder

    r = db.execute(update(AcquisitionOrder)
                   .where(AcquisitionOrder.id == order_id, AcquisitionOrder.status == was)
                   .values(**values))
    return r.rowcount == 1
...
def poll_orders() -> dict:
    ...
    with SessionLocal() as db:
        ...
        for o in rows:
            ...
            # живой execute («↻ повторить»). Ни одно решение не уезжает в БД слепым UPDATE'ом по
            # первичному ключу — только вместе с этим `was`.
            was = o.status
            d = db.get(Domain, o.domain_id)
            ...
            try:
                if not _settle(db, o.id, was, **values):
                    ...
                    continue                          # ушла в ordered/cancelled — это видно в очереди
