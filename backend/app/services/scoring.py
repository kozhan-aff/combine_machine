...
def _deadline_from_whois(existing, free_date, now, lane):
    ...
    # Обновляем ТОЛЬКО просроченную проекцию и ТОЛЬКО вперёд, и НИКОГДА для bid/free —
    # у обоих «дедлайн прошёл, а домен занят» есть СВОЙ законный терминал, и переписывать
    # его чужим free-date значит отобрать у домена терминальное состояние вовсе:
    ...
    #          acquirability_verdict: `taken` заслуживает именно lane='free'). Без этого
    #          исключения перекупленный домен получал бы free-date НОВОГО владельца на год
    #          вперёд, вердикт waiting, и висел бы в инбоксе кандидатом к выкупу вечно.
    # Целевая популяция находки — lane=NULL (бездедлайновый пул) — не задета.
    if lane not in ("bid", "free") and now > ex + DROP_GRACE and new > ex:
        return new
    return existing
