...
- **Optimizator.ru** — второй канал выкупа доменов (nic.ru/RU-CENTER), денежный гейт,
  ...
  блокер, не код).
- **Живой аудит бокса** (2026-07-17, с Mac через Basic-auth): все интеграции здоровы
  кроме ожидаемо недоступного с Mac aaPanel (IP-whitelist). **Кнопка «⟳ Перепроверить
  занятость» подтверждённо работает** (43 домена: 4 свободных, 37 ждут дропа, 1 занят,
  1 не определился). На боксе: `офферы=0, куплено=0, сайтов=0, черновики=0, edited=0,
  опубликовано=0` — пайплайн НИ РАЗУ не пройден дальше M1-одобрения. **Реальный
  блокер MVP — операционный, не код:** нет ни одного оффера (без него M4 писать не о
  чем) и нет денег ни в одном из двух каналов выкупа.
