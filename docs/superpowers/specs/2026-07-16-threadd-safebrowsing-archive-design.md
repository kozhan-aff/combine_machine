...
> **Пост-ревью поправка (2026-07-16, ночь).** Финальное whole-branch ревью (combine-
> reviewer, opus) нашло: Archive-часть этого дизайна (пункт 2 «Решённое в диалоге» и
> вся секция 2 «Архитектуры» ниже) **не работает и не нужна** — оставлено ниже как
> есть, ЗАЧЕРКНУТО фактически (не буквально `~~~~`, штатной Markdown-разметки
> вычеркивания в этом файле не используем — просто читай с поправкой) кодом-коммитом
> `b6d50d0`, который её убрал целиком. Две независимые причины:
> 1. `_parse_archive` мапит буквальную строку `"none"` (то, чем API ВСЕГДА отвечает на
>    честно пустой архив — ни разу не видели литерал `"0 times"` ни на одном живом
>    примере) в Python `None`, а не в `0`. Условие `skip_wayback = arch.get("times")
>    == 0` поэтому НИКОГДА не становится `True` — весь механизм мёртв с рождения.
> 2. Даже если бы условие срабатывало — экономии всё равно нет: `wayback.
>    classify_history` УЖЕ выходит мгновенно, когда снимков нет (`get_snapshots`
>    вернул `[]`) — дорогой цикл `_fetch_raw`+`_visible_text`+`sleep` (источник тех
>    самых ~60с/домен) запускается ТОЛЬКО когда снимки РЕАЛЬНО ЕСТЬ, а именно тогда
>    Archive-гейт и не должен ничего пропускать по замыслу. Пропускать было
>    физически нечего.
>
> Итог: SafeBrowsing hard-reject (пункт 1, вся остальная секция) — реализован,
> проверен, смержен без изменений, ревью его одобрило полностью. Транспортные методы
> `archive_probe`/`_parse_archive`/пресет `no_proxy` (секция «Архитектура», первый
> код-блок) остались в `aparser.py` как рабочий, живьём протестированный код БЕЗ
> вызывающего кода в воронке — просто больше ничего их не зовёт. Читай секции ниже
> как «как это задумывалось», а не «как это работает сейчас» для Archive-части.
