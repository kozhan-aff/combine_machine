...
Панель `:8000` **выставлена на LAN** (`0.0.0.0:8000:8000`) — открывается с Mac по
`http://192.168.1.77:8000/`. Авторизации у панели нет, поэтому это ОК только за NAT
домашнего роутера: **не пробрасывать `:8000` в интернет**. Хочешь приватно — верни
`127.0.0.1:8000:8000` и ходи через SSH-туннель (§3). Postgres `:5432` остаётся
loopback-only. (Docker публикует порт в обход хостового фаервола — работает даже при
default-deny ufw на боксе.)
