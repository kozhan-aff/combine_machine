...
скачать «сырой» HTML (`id_`) → **вырезать разметку и оставить ВИДИМЫЙ ТЕКСТ + `<title>`**
(`_visible_text`, nh3) → прогнать по ключевикам → выставить флаги. Считать стоп-слова в сырой
разметке нельзя: два «casino» в `<script>` рекламной сети или в `alt=` картинки — это не история
домена, а мебельный магазин, отбракованный как казино (аудит 2026-07-14, F3).
