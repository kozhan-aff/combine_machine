...
Все — `GET .../manager/billmgr?func=...&out=json&authinfo=$login:$password`. Ответы authed-функций
без валидных кредов вернуть не удалось (нет тестового аккаунта) → форма ответа ниже по офиц. доке;
точная обёртка верхнего уровня (billmgr обычно `{"doc":{...}}`) — **UNVERIFIED**, доверяем указаниям
доки, что нужные поля лежат по путям `elem[0].id` / `elem[n].id`.
